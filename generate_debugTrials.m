...
% --- Updated Section: Exact naming format matching ---
    % Expected format: SubjID_TaskType_ElbowPosture_90_pair_XX_block_XX_ACTIVE.mat
    fileName = fullfile(outDir, ...
        [subjID '_' taskType ...
        '_ElbowPosture_90' ...
        '_pair_' sprintf('%02d', b) ...   % Injected Pair ID
        '_block_' sprintf('%02d', b) ...  % Keep block tracking aligned
        '_ACTIVE.mat']);
    
    save(fileName, 'TrialStim_param');
    disp(['Saved: ' subjID '_' taskType '_ElbowPosture_90_pair_' sprintf('%02d', b) '_block_' sprintf('%02d', b) '_ACTIVE.mat']);
end
end

function generate_trials_MotionDirection_with_passive(parDir, subjID, taskType) % Generates all active trials 
...
VarNames = { ...
    ...
    'EncoderSamples', 'OnsetSample', 'AbsExpStart', 'AbsTrialStart', 'isRep', 'Cued_Speed','StimMoveUp' ...
};
...
VarType = { ...
    ...
    'double', 'double', 'double', 'double', 'double', ...
    'double', 'double', 'double', 'double', 'double', ...
    'cell',   'cell',   'double', 'double', 'double', 'double', 'double' ...
};
% =========================
...
for post = 1:length(armPosture)
    ...
    for bb = 1:n_active_blocks
        ...
        TrialStim_param = table('Size', [nTrials length(VarNames)], ...
            'VariableNames', VarNames, 'VariableTypes', VarType);

        for n = 1:nTrials

            armSpeedAbs = blockRows(n,3);
            stimDir = blockRows(n,4);
            armSign = blockRows(n,6);

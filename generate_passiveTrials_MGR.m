...
passiveBlocks.Arm_Mov_Speed_arduino(motionTrialsIdx) = round(passiveBlocks.Arm_Mov_Speed(motionTrialsIdx) ./ cm_per_step);
passiveBlocks.Arm_Mov_Acc_arduino(motionTrialsIdx) = round(abs(passiveBlocks.Arm_Mov_Speed_arduino(motionTrialsIdx)) .* 1000);
passiveBlocks.Arm_Mov_Steps_arduino(motionTrialsIdx) = negativeVelocity .* round((passiveBlocks.Arm_Mov_Speed(motionTrialsIdx) .* t) ./ cm_per_step);
passiveBlocks.Arm_Mov_StepsAbs_arduino(motionTrialsIdx) = abs(passiveBlocks.Arm_Mov_Steps_arduino(motionTrialsIdx));
